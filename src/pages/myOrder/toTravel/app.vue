                        ...
                        <div class="wayItem" v-for="(item, index) in orderDate" :key="index" @click.stop="goDetail(item.orderSort, item.channelOrderNo, item.orderNo)">
                            ...
                            <div class="cont flexbox" v-if="item.orderSort == 'Upgrade'">
                                ...
                                <div class="ow-main flex-1"
                                    v-for="(key, index) in item.segmentInfoList" 
                                    :key="index" 
                                    v-if="key.flightDirection == 'G' && (key.changeSegment == 'upgrade' || !key.changeSegment)">
                                ...
                                <div class="ow-main flex-1"
                                    v-for="(key, index) in item.segmentInfoList" 
                                    :key="index" 
                                    v-if="key.flightDirection == 'B' && (key.changeSegment == 'upgrade' || !key.changeSegment)">
                                    <div class="ow-hd">{{ key.formatCabinClassName }}({{ key.formatCabin }}) 升至 {{ key.cabinClassName }}({{ key.cabin }})</div>
                                    <div class="ow-bd">{{ turnTime(key.depDateTime, 'MM-dd hh:mm')  }} 至 {{ turnTime(key.arrDateTime, 'MM-dd hh:mm') }}</div>
                                    <div class="ow-ft">
                                        <span>{{ key.flightNo }}</span>|
                                ...
                                <div class="ow_rt">
                                    <p class="p1">{{ currencyType(item.currencyCode) }}<span>{{ item.amount + (item.currencyCode == 'JPY' ? '円' : '') }}</span></p>
                            ...
                            <div class="cont flexbox" v-if="item.orderSort == 'Change'">
                                ...
                                <div class="ow-main flex-1"
                                    v-for="(key, index) in item.segmentInfoList" :key="index"
                                    v-if="key.flightDirection == 'G' && key.changeSegment == 'change'">
                                    <div class="ow-hd">{{ key.depCityName }} - {{ key.arrCityName }}<span>(改期订单)</span></div>
                                    <div class="ow-bd">{{ turnTime(key.depDateTime, 'MM-dd hh:mm')  }} 至 {{ turnTime(key.arrDateTime, 'MM-dd hh:mm') }}</div>
                                    <div class="ow-ft">
                                        <span>{{ key.flightNo }}</span>|
                                        <span>{{ (key.depCityName?key.depCityName:'') + (key.depAirportName?key.depAirportName:'')  + ((key.depTerm == "--" || key.depTerm == null) ? "" : key.depTerm) }}出发</span>
                                ...
                                <div class="ow-main flex-1"
                                    v-for="(key, index) in item.segmentInfoList" 
                                    :key="index" 
                                    v-if="key.flightDirection == 'B' && key.changeSegment == 'change'">
                                    <div class="ow-hd">{{ key.depCityName }} - {{ key.arrCityName }}<span>(改期订单)</span></div>
                                    <div class="ow-bd">{{ turnTime(key.depDateTime, 'MM-dd hh:mm')  }} 至 {{ turnTime(key.arrDateTime, 'MM-dd hh:mm') }}</div>
                                    <div class="ow-ft">
                                        <span>{{ key.flightNo }}</span>|
                            ...
                            <!-- 机票订单(原订单) -->
                            <div class="cont flexbox" v-if="item.orderSort != 'Change' && item.orderSort != 'Upgrade'">
                            ...
                            <div class="btns flexbox">
                                ...
                                <div class="ow_del" 
                                    v-if="item.orderPayState=='Pay' && 
                                    dateFormat.formatIOSDate(item.segmentInfoList[0].depDateTime) < new Date()"
                                    @click.stop="evalOpen(item)">评价</div>
                                <!-- 行程单不能取消 -->
                                <div class="ow_del" 
                                    v-if="item.orderPayState == 'UnPay' && item.orderSort!='TicketDeliveryType'"
                                    @click.stop="cancel(item.channelOrderNo, item.orderNo, item.channelCode, item.orderSort)">取消订单</div>
                                <!-- 升舱去支付 -->
                                <div class="ow-btn" v-if="item.orderSort == 'Upgrade' && item.orderPayState == 'UnPay' && item.currencyCode=='CNY'" @click.stop="goPay(item.orderSort, item.channelOrderNo, item.orderNo, item.payType)">去支付</div>
                                <!-- 改期去支付 -->
                                <div class="ow-btn" v-if="item.orderSort == 'Change' && item.orderPayState == 'UnPay' && item.currencyCode=='CNY'" @click.stop="goPay(item.orderSort, item.channelOrderNo, item.orderNo, item.payType)">去支付</div>
                                <!-- 机票订单去支付 -->
                                <div class="ow-btn" v-if="item.orderSort != 'Change' && item.orderSort != 'Upgrade' && item.orderPayState == 'UnPay' && item.currencyCode=='CNY'" @click.stop="goPay(item.orderSort, item.channelOrderNo, item.orderNo, item.payType)">去支付</div>
                                <div class="ow_del" v-if="item.orderPayState == 'Cancel'" @click.stop="del(item.channelOrderNo, item.orderNo)">删除订单</div>
                                <div class="ow_del" 
                                    v-if="item.orderPayState == 'Pay' && 
                                    item.orderSort != 'Change' && 
                                    item.orderSort != 'Upgrade' && 
                                    dateFormat.formatIOSDate(item.segmentInfoList[0].depDateTime) > new Date()"
                                    @click.stop="goChangeDate(item.channelOrderNo, item.orderNo)">改期</div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    import { getChannelCode } from 'common/logic/utils'
    ...
    export default {
        ...
        methods: {
            ...
            createParam(pageNum) {
                ...
                return {
                    request: {
                        ...
                        dateBegin: dateFormat.getToday(),
                        dateEnd: dateFormat.getToday(0, 6),
                        pageNo: pageNum,
            ...
            getOrderList(pageNum) {
                ...
                controller.getJSON({
                        method: 'POST',
                        url: requesturl.MY_ORDER.AIR_TRAVEL,
