			...
			<div class="main-wrap">
				...
				<div class="content-wrap">
					<div class="select-ticket-wrap com-wrap-style">
						<div class="title">选择抵用券</div>
						<div class="content clearfix" >
							<div class="every-select" @click='selectTicket(index)' :class='{"active": curSelectIndex == index, "com-border": curSelectIndex != index}' v-for="(item, index) in ticketInfo" :key='index'>
									<div class="wrap flexbox justify-content flex-v">
										<div class="weight">{{item.couponPrice}}KG</div>
					...
					<div class="concat-people-infor com-wrap-style">
						...
						<div class="every-line flexbox">
							...
							<div class="right-wrap com-input-style-1 flex-1">
								<input type="number" pattern="[0-9]*" placeholder="请输入手机号" v-model="orderPhone">
							</div>
						</div>
					</div>
					...
					<div class="detail-page" v-show='displayDetail' @click='clickDetailBtn'>
						<div class="detail-wrap">
							<div class="commodity-title every-detail  flexbox" :class="{'com-border-bottom' : scoreObj.useScore}">
								<div class="txt flex-1">行李券-{{curSelectTick.couponPrice}}KG</div>
...
import { ChannelOrderNo, vilidation, mul2 } from "common/logic/utils";
import {
  ResultCode,
  AuthType,
  RegEnum,
  ...
  methods: {
    ...
    getLuggageInfor() {
      ...
      controller
        .getJSON({
          method: "POST",
          url: requesturl.LUGGAGE.LUGGAGE_INFOR,
          params: that.luggageParams()
        })
        .then(that.lunggageSucBack)
        .catch(that.lunggageErrBback);
    },
    luggageParams() {
      return {
        request: {
    ...
    lunggageSucBack(data) {
      ...
      if (data.resultCode == ResultCode.SUCCESS) {
        ...
        this.orderName = objData.linkName ? objData.linkName : this.orderName;
        this.orderPhone = objData.linkPhone
          ? objData.linkPhone
          : this.orderPhone;
    ...
    orderParams() {
      return {
        request: {
          ...
          linker: this.orderName,
          linkerHandphone: this.orderPhone,
          ...
          couponSource: couponType.LUGGAGE
        }
      };
    },
    orderSucBack(result) {
    ...
    startVerification() {
      if (!!this.orderName) {
        if (!this.vertificate(this.orderName, RegEnum.NOT_STRICT_NAME, 2000)) {
          return false;
        }
      } else {
        controller.toast({
      ...
      if (!!this.orderPhone) {
        if (!this.vertificate(this.orderPhone, RegEnum.MOBILE, 2000)) {
          return false;
        }
      } else {
        controller.toast({
    ...
    // 点击购买须知按钮
    clickRulesBtn() {
      let buyRulesPage = Page.luggage.buyRules;
      controller.open({
