    ...
    <div id="app">
        ...
        <div class="main" :class='{mainHasBottom : !isDetailPage}'>
            ...
            <div class="container">
                ...
                <!-- 选择服务 -->
                <div class="post-wrapper">
                    <div class="writeInformation-title">选择服务</div>

                    <ul>

                        <li class="every-insurance clearfix" v-if="isDetailPage ? isDetailPage && psgInfo.serviceType === wchcType.WCHR : true">
                            <div class="label-content flexbox">
                                <div class="insurance-information flex-1" v-bind:style="{ 'max-width': linkStyle}">
                                    <div>
                                        <span class="name">地面轮椅</span>
                                    </div>
                                    <div class="introduce">身体适宜乘机，能自行上下飞机，在客舱内能自己走到座位上，仅在航站楼、停机坪与飞机之间需要轮椅服务的旅客</div>
                                </div>
                            </div>
                            <label class="select-checkbox clearfix" v-show="!isDetailPage">
                                <input type="radio" name="wchcName" :value="wchcType.WCHR" v-model="psgInfo.serviceType" class="label-checkbox">
                                <span class="select-btn"></span>
                            </label>
                        </li>

                        <li class="every-insurance clearfix" v-if="isDetailPage ? isDetailPage && psgInfo.serviceType === wchcType.WCHS : true">
                            <div class="label-content flexbox">
                                <div class="insurance-information flex-1" v-bind:style="{ 'max-width': linkStyle}">
                                    <div>
                                        <span class="name">登机轮椅</span>
                                    </div>
                                    <div class="introduce">身体适宜乘机，不能自行上下飞机，但可以在客舱内自己走到座位上的旅客</div>
                                </div>
                            </div>
                            <label class="select-checkbox clearfix" v-show="!isDetailPage">
                                <input type="radio" name="wchcName" :value="wchcType.WCHS" v-model="psgInfo.serviceType" class="label-checkbox">
                                <span class="select-btn"></span>
                            </label>
                        </li>

                        <li class="every-insurance clearfix" v-if="isDetailPage ? isDetailPage && psgInfo.serviceType === wchcType.WCHC : true">
                            <div class="label-content flexbox">
                                <div class="insurance-information flex-1">
                                    <div class="all-inputBox">
                                        <span class="name">机上轮椅</span>
                                    </div>
                                    <div class="introduce">身体适宜乘机，但在客舱内无行走能力的旅客</div>
                                </div>
                            </div>

                            <label class="select-checkbox clearfix" v-show="!isDetailPage">
                                <input type="radio" name="wchcName" :value="wchcType.WCHC" v-model="psgInfo.serviceType" class="label-checkbox">
                                <span class="select-btn"></span>
                            </label>
                        </li>
                ...
                <!-- 接机信息 -->
                <div class="post-wrapper" v-show="ifjj">
                    <div class="writeInformation-title">接机人信息(选填)</div>
                    <ul>
                        <li>
                            <label class="flexbox">
                                <div class="all-inputBox">
                                ...
                                <input type="text" :readonly='isDetailPage' placeholder="请输入接机人姓名" class="all-input-style flex-1" v-model="psgInfo.meetName">
                            </label>
                        </li>
                        <li>
                            <label class="flexbox">
                                ...
                                <input type="tel" pattern="[0-9]*" :readonly='isDetailPage' placeholder="请输入接机人手机号" class="all-input-style flex-1" v-model="psgInfo.meetPhone">
                            </label>
                        </li>
                    </ul>
                </div>
                <!-- 送机信息 -->
                <div class="post-wrapper" v-show="ifsj">
                    <div class="writeInformation-title">送机人信息(选填)</div>
                    <ul>
                        <li>
                            <label class="flexbox">
                                <div class="all-inputBox">
                                ...
                                <input type="text" :readonly='isDetailPage' placeholder="请输入接机人姓名" class="all-input-style flex-1" v-model="psgInfo.sendName">
                            </label>
                        </li>
                        <li>
                            <label class="flexbox">
                                ...
                                <input type="tel" pattern="[0-9]*" :readonly='isDetailPage' placeholder="请输入送机人手机号" class="all-input-style flex-1" v-model="psgInfo.sendPhone">
                            </label>
                        </li>
                    </ul>
                </div>
    ...
    export default {
        data() {
            return {
                ...
                // 到达机场的三字码
                endAirportEName: '',
                rulesData: SpecialTravellerRules.WCHC,
                canShowRules: false,
                //图片地址存储
                imgArr: [],
                //附件地址
        ...
        computed: {
            submitBtnCls() {
                if (this.checkInput && this.psgInfo.passengerName && this.psgInfo.certType && this.psgInfo.certNo &&
                    ...
                    if(this.psgInfo.accompanyName && this.psgInfo.accompanyHandphone || (this.psgInfo.accompanyName == "" && this.psgInfo.accompanyHandphone == "")){
                        this.canClickBtn = true;
                    }else{
                        this.canClickBtn = false;
                        return 'g-disabled' 
                    ...
                    if(this.psgInfo.meetName && this.psgInfo.meetPhone || (this.psgInfo.meetName == "" && this.psgInfo.meetPhone == "")){
                        this.canClickBtn = true;
                    }else{
                        this.canClickBtn = false;
                        return 'g-disabled' 
                    ...
                    if(this.psgInfo.sendName && this.psgInfo.sendPhone || (this.psgInfo.sendName == "" && this.psgInfo.sendPhone == "")){
                        this.canClickBtn = true;
                    }else{
                        this.canClickBtn = false;
                        return 'g-disabled' 
        ...
        methods: {
            ...
            seeDetails() {
                ...
                .then((res) => {
                    ...
                    if (res.resultCode === ResultCode.SUCCESS) {
                        ...
                        this.psgInfo.serviceType = res.objData.speServiceType
                        ...
                        this.linkStyle = 'inherit'
                        if(res.objData.attachment != null){
                            let str = res.objData.attachment
                            this.imgArr = str.split(",")
                        }

                        if(this.psgInfo.accompanyName!=null && this.psgInfo.accompanyHandphone!=null ){
                            this.ifpt = true;
                        }else{
                            this.ifpt = false;
                        }
                        if(this.psgInfo.sendName!= null && this.psgInfo.sendPhone!= null ){
                            this.ifsj = true;
                        }else{
                            this.ifsj = false;
                        }
                        if(this.psgInfo.meetName!= null && this.psgInfo.meetPhone!=null ){
                            this.ifjj = true;
                        }else{
                            this.ifjj = false;
                        }
            ...
            submitApply() {
                ...
                if(that.canClickBtn && that.valify()){
                    ...
                    if(that.imgArr.length == 0){
                        controller.getJSON({
                            method: 'POST',
                            url: requesturl.PASSENGERS.WHEELCHAIR,
                        ...
                        .then((res) => {
                            ...
                            if (res.resultCode === ResultCode.SUCCESS) {
                                ...
                                controller.open({
                                    ...
                                    data: {title: that.title, type: SuccType.APPLY_SUCC, pageName: Page.passengers.wchc.winName, id: id},
                                    closeLastPage: true
                                });
                            } else {
                                controller.toast({str: res.resultInfo});
            ...
            createParams() {
                ...
                return {
                    "request": {
                        ...
                        "serviceType": _that.psgInfo.serviceType,
            ...
            valify() {
                ...
                //陪同人信息验证
                if(this.psgInfo.accompanyName != "" && this.psgInfo.accompanyHandphone != ""){

                    if(this.psgInfo.accompanyName.length > 25){
                        controller.toast({str: Verify.BACK_ERR.toastNameLen});
                        return false;
                    }
                    if(!this.vertificate(this.psgInfo.accompanyName, RegEnum.NAME_ALL, 2000)) {
                        return false;
                    }
                    if(!this.vertificate(this.psgInfo.accompanyHandphone, RegEnum.MOBILE, 2000)){
                        return false;
                    } 
                ...
                //接机人信息验证
                if(this.psgInfo.meetName != "" && this.psgInfo.meetPhone != ""){
                ...
                //送机人信息
                 if(this.psgInfo.sendName != "" && this.psgInfo.sendPhone != ""){
